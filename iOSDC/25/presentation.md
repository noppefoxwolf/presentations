...
^ 余談になりますが、先日iOS26のデザイン対応やImage Playgroundの導入の観点でAppStoreでのフィーチャーを獲得しました。
...
^ ちなみに、Mastodonを使っている方どれくらいいますか？Mastodonをご存知ない方のために、まず簡単に説明させていただきます。
...
^ メモリ使用量は、一般的な8bitのARGB画像の場合、M=W×H×C×Nという式で計算できます。
...
^ 当然、無条件に処理が軽くなるのと嬉しいのですが、突き詰めると重要でないものの品質を落とし、重要なものの品質を上げるという話になります。このとき、「ユーザー体験」と「アプリのコアバリュー」を軸に取捨選択を行います。

^ つまり、極論いくらメモリやCPUが使われても、ユーザーが快適と感じるなら問題無いと割り切ってしまって良いということです。
^ エンジニアは見えているパフォーマンス問題を解決したくなりますが、それを直して誰が嬉しいのかを考えると、優先度をつけやすくなるかなと思います。
...
^ 今回はInstrumentsの使い方などの詳細はこのトークではお話ししませんが、この辺りに興味がある方がいればトーク後にお声がけいただければと思います。

^ また、WWDCでも多くのセッションで紹介されているので、ぜひ活用してみてください。
...
^ これにより、スライドのように複数のアニメーション絵文字を別々にロードしても同じタイミングで動作するようになります。
^ ミーム系のGIF画像など、複数の同じ画像が表示される場合や、GIFを並べることで意味が出る画像を表示する場合に効果的です。
...
^ 先ほど紹介したUpdateLinkですが、内部的にはUIUpdateLinkで実装しています。
^ UIUpdateLinkは画面の更新タイミングに合わせてコードを実行できるiOS17の新機能です。
^ 時間の経過をベースにしているタイマーと違って、画面描画と同期するため、毎回の処理の猶予時間が安定して扱いやすくなります。
...
^ リサイズの処理コストが高いため、実際にはこの工程ではリサイズ後のサイズだけを決めて、次の工程に進みます。
...
^ この時点で、全てのフレームをデコードした場合に使われるメモリの量が計算できるので、それが大きすぎる場合はフレームを間引いて調整します。
...
^ 左は、単純に後半のフレームをスキップしたもの、右は、均一に間引いたものです。
...
^ 実際に調整している様子がこちらです。
^ 画面下にあるスライダーはintegrityというパラメータで、1.0がオリジナルのフレーム数、0.0が最低限のフレーム数になります。
^ integrityを調整することで、フレームレートが変化していますが、どの値を取ってもアニメーションの連続性は保たれています。
...
^ ところで、ここで一つ問題があります。
^ 元々表示サイズより小さな、リサイズの必要が無いフレーム画像でも、必ず各フレーム画像をレンダリングする必要はあるのでしょうか？
^ 答えは、YESです。
^ 実は次のような理由があります。
...
^ CGImageSourceから取り出されたフレーム画像を、UIImageとして表示するとDGifDecompressという処理が発生します。
^ これは、CGImageSourceから取り出したGIFのCGImageが圧縮されており、そのままでは描画に使用できないため復元の処理をしています。
^ この処理がデフォルトの挙動では、UIImageは描画の直前までフレームを復元しないという挙動になっています。
^ つまり描画の直前ということは、このそこそこ重い復元処理がメインスレッドで行われるということです。
^ これが、スクロールの引っ掛かりの原因になっていました。
...
^ どちらもバックグラウンドスレッドで実行できるため、事前に処理しておくことで画面に表示される瞬間に重い処理が走ってスクロールがカクつくのを防げます。
...
^ AnimatedImageでは、これらのトレードオフをパラメータで調整できるようにすることでクオリティコントロールを可能にしています。

---

![](oss.png)

^ これで以上になるのですが、本日ご紹介したAnimatedImageをはじめ、DAWN for Mastodonは主要な機能を30を超えるOSSとして公開しています。
...
^ 今回のトークの資料を作る中でも、まだまだ最適化の余地を見つけているので、今後も継続的に改善していきたいと考えています。
^ ぜひ、AnimatedImageを使ってみて、フィードバックをいただけると嬉しいです。

^ また、今回登場したWebP自体の細かい仕様については、午後の岸川さんの「作って学ぶWebP入門」をご覧いただくと、より理解が深まるのではないでしょうか。
