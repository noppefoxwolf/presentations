...
^ 具体的な見た目の前に、UIを三つの要素に分けて考えます。ユースケース、オブジェクト、インターフェースです。
...
^ ユースケースはUIの目的です。「誰が、何を、どうできるか」と言い換えてもよいです。
...
^ オブジェクトは、そのユースケースにおける「何を」です。天気、アプリ、ドキュメント、メッセージのような対象を指します。
...
^ 広い言葉ですが、今日は「最終的にユーザーがアプリと接点を持つ部分」として扱います。
...
^ 例えばホーム画面です。ユースケースは「ユーザーがアプリを選択して起動できること」。オブジェクトは「アプリ」。そして、グリッドにアプリのボタンが並ぶのがインターフェースです。

---

# インターフェースは複数存在することができる

^ ここで大事なのは、ユースケースとオブジェクトが同じでも、インターフェースは一つに決まらないことです。
...
^ ホーム画面も同じです。通常はグリッドですが、アシスティブアクセスでは大きなボタンになり、VoiceOverではアクセシビリティ階層として扱われます。Siriなら音声から直接アプリを開けます。
^ どれも「アプリを選んで起動する」という同じ目的に応えています。インターフェースは、必ずしも目に見えるUIである必要すらありません。
...
^ そこで、個別の画面を増やす代わりに、二つの性質を使います。AdaptivityとFlexibilityです。
...
^ たとえば幅や文字量のように連続的に変わるものはFlexibilityで扱えます。一方、iPhoneからiPadのように性質が変わる境目では、Adaptivityで切り替えます。
...
^ そんなときは伸ばし続けるのではなく、Adaptivityで構造や表現そのものを切り替えます。
...
^ Adaptivityの代表的な手段は、省略・拡張、折り畳み・展開、そしてレイアウトの切り替えです。ここでも、ユースケースとオブジェクトは保ちます。
...
^ 状況に応じて表現を切り替えているので、ここにはAdaptivityがあります。それでも、伝えたいテキストというオブジェクトとユースケースは変わりません。
...
![画像プレースホルダー：幅が広がる間はカードが比率を保って伸び、しきい値を超えると最大幅で止まり、2カラムへ切り替わる連続→離散の図](assets/placeholder-flexibility-adaptivity-combo.png)
...
^ 途中までは比率や余白をFlexibilityで受け止め、過剰になったところで最大サイズに止める、または別のレイアウトへ切り替える。そこにAdaptivityを使います。
...
^ UIKitには、構造レベルのAdaptivityを持つコンテナビューコントローラが標準で用意されています。
...
^ さらに性質の変わる境界を見つけたら、そこでAdaptivityを使います。iPhoneとiPadだけでなく、iPhoneの縦横で切り替えてもよい、ということです。

---

# ここまで

状態ごとのUIと、Flexibility／Adaptivityを理解した

^ ここまでで、状態によってインターフェースが決まり、FlexibilityとAdaptivityで柔軟性を作る、という話をしました。
