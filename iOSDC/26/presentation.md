...
^ これは、iPadOSだからというわけではなく、macOS Golden GateからはiOSのアプリも、iPhone Mirroringでリサイズできるようになりました。

---

![](8.png)

^ さらに、UISceneDelegateの移行により、アプリが単一の固定された画面だけを持つ、という前提も薄れているように思います。

---

![](J0000050994.jpg)

^ そして、これまで縦長だったiPhoneの画面も、先日登場したiPhone Ultraの登場により、「iOSアプリは縦長」という前提は崩れてきたのかなと思います。
...
^ このように、今日題材となっている「UIに柔軟性を持たせる」とは、ユーザーが置かれている状況によらずユーザーの「やりたい」に応えることです。
...
^ さて、前半では、UIの柔軟性を持たせるためには、インターフェースをコンテキストに応じて変える必要があることを話しました。
^ そして、その応答には、同じ構造や表現のまま追従する連続的な適応と、境界で構造や表現を切り替える段階的な適応があることも話しました。
^ これを実際にUIのレイアウトで活用してみましょう。
...
^ これが、前半で話した、境界でインターフェースの構造や表現を切り替える段階的な適応です。
