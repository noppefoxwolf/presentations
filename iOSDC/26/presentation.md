...
^ 次に、これらの要素のうち、どれが柔軟性を持つことに適した要素なのかを考えてみます。
^ ３つの要素うち、２つを固定して、残りの１つがどんな柔軟性を要求されるか観察してみます。
...
^ つまり理想のインターフェースは目的とコンテキストから導出することができると考えることもできます。
^ 実際には、ケースごとに実装していくことにはなりますが、考え方としてはこのように留めておくことで、コンテキストのバリエーションからインターフェースに別の姿があるのではないかなと疑えるようになるのではないかなと思います。
...
^ 境界があるかどうかは、同じインターフェースのまま目的を十分に達成できるかで判断します。
