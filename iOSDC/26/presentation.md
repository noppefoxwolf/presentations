...
^ これは、iOSアプリがiPhoneのシーンだけを用意するのではなく、環境に合わせて複数のシーンを用意することが前提になってきた、ということです。
...
^ Apple Musicを例に上げてみましょう。この画面の目的は「プレイリストを選ぶこと」です。
^ この時、インターフェースはグリッドとリストの2種類が存在します。
^ これはユーザーの「アートワークで覚えている」だったり、「プレイリスト名で覚えている」というコンテキストによって選択されます。
...
^ そこで、コンテキストの変化への対応方法を２つに分けて考えてみます。
^ 連続的に変化するものと、境界があるものです。
...
^ 連続的に変化するコンテキストに対しては、インターフェースは構造や表現を変えずに伸縮することで対応します。
^ たとえば、ビューのサイズや文字量のように連続的に変化するものは、構造や表現を変えずに伸縮することで対応します。
...
^ 一方で、境界を持つコンテキストは、UIの構造や表現そのものを切り替えることで対応します。
...
^ ある幅までは1カラムで目的を満たせるデバイスも、さらに広くなったら2カラムの方が目的達成に適するとなると、そこに境界があることがわかります。
^ 画面に映るGUIなのか、音声で操作するVUIなのかも、このような境界を持つコンテキストの例です。どちらも同じ目的を達成できますが、インターフェースは大きく変わります。
...
^ コンテキストが境界を持つかどうかは、コンテキストが目的に影響を与えるかどうかで判断します。
^ 例えば、iPhoneしかない状況では、カーソル操作のUIは適さないので、境界があります。
^ 一方で、目的に寄りますがユーザーの身長は、UIの構造や表現に影響を与えないので、境界はないと言えます。
...
^ かなり概念的な話になってしまいましたが、後半はGUIに注目して、このコンテキストの性質をレイアウトの話に落とし込む話をしていきます。

---

# 連続的レイアウトと段階的レイアウト

^ さて、前半では、UIの柔軟性を持たせるためには、インターフェースをコンテキストに応じて切り替える必要があることを話しました。
^ そして、コンテキストには連続的に変化するものと、境界を持つものがあることも話しました。
...
^ これが、前半で話した、境界を持つコンテキストに応じたインターフェースの切り替えです。
