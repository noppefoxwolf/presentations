...
^ このように、今日題材となっている「UIに柔軟性を持たせる」とは、ユーザーが置かれている状況によらず「やりたい」に応えることです。
...
^ この図で言うと、右がユーザーで、左がアプリです。
^ この時、アプリとユーザーの仲介役として、中央のUser Interfaceが存在しています。

---

![](9.png)

^ ここで、関連する要素を抜き出してみると、ユーザーが行うアプリの操作には「目的」「インターフェース」「コンテキスト」の3つが関係していることが分かります。
...
^ ３つのうち、２つを固定したときに、残りの１つは柔軟性を要求されることが分かります。
...
- コンテキスト：車を止めて、iPhoneを手に取る

^ 目的とインターフェースが固定されている場合、コンテキストを変える必要があります。
^ 例えば、目的は「アプリを起動すること」、インターフェースは「グリッドにアプリのボタンが並ぶこと」と固定されている場合、仮にユーザーが運転中の場合は操作をすることができず、ユーザーは「車を止める」というコンテキストを変える必要があります。
...
^ 目的は「アプリを起動すること」、コンテキストは「車を運転していること」と固定されている場合、アプリはインターフェースを変える必要があります。
...
^ 目的は「車を運転していること」、インターフェースは「グリッドにアプリのボタンが並ぶこと」と固定されている場合、ユーザーは音楽が聴きたいのに、アプリを起動することしかできず、目的を変える必要があります。
...
^ このように、複数のインターフェースを提供することで、ユーザーは現在のコンテキストを変えずに目的を達成することができます。
