...
^ そのために、３つの要素のうち２つを固定して、残りの１つにどんな変化が要求されるのかを実験してみます。
...
^ 最初は、「アプリを起動したい」という目的と「画面に並んだボタンで操作する」インターフェースの２つの制約があるとします。
...
^ 次は、固定する組み合わせを変えます。
^ 「アプリを起動したい」という目的と「運転中である」というコンテキストが固定されているとします。
...
^ 最後は、「画面に並んだボタンで操作する」というインターフェースと「運転中である」というコンテキストを固定します。
...
^ このように、UIの３つの要素のうち、どれに制約があるのかによって、変化を要求される要素が決まります。
^ さて、皆さんはどの要素が変化するのがユーザーにとって望ましいと思いますか。
...
^ ここで重要なのは、アプリに対してインターフェースは複数存在することができる、ということです。
^ 実際のアプリで見てみましょう。
...
^ 先ほどの考え方に当てはめると、「デザインデータ」に描かれているインターフェースとは、ある特定のコンテキストの中で、ある特定の目的を達成するためのインターフェースの一つに過ぎません。
^ なので、デザインデータをそのまま実装すると、柔軟性がないUIになってしまいます。つまり、インターフェースを固定してしまっていることになります。
...
^ では実際にアプリがコンテキストに応じてインターフェースを変えるには、アプリが今どんなコンテキストで使われているのか知る必要があります。
^ まずは一度、コンテキストが「どこで生まれているか」という観点を持ってみましょう。
...
^ しかしiPhoneのアプリの柔軟性を考える上で、MacやWatchのように大きく異なるデバイスをいきなり考える必要はありません。
...
^ 枠のサイズが変わったとき、中身を枠に合わせるのか、それとも中身のサイズを優先するのか。
^ と言った２つの選択肢があるので、この２つを見ていきます。
...
^ そこで、ビュー自身のサイズを優先するときも、固定の数値ではなく、コンテンツに必要なサイズを基準にします。
...
^ この方法を使うと、画面サイズがどんなに変化しても同じUIを伸縮させられます。
...
^ ここでの優先度は、目的から逆算して決めます。たとえば、音楽オブジェクトの目的は「曲を選ぶこと」とした時に、アートワーク、曲名、アーティスト名の中で、どれが重要かを考えます。
...
^ 同じ設定画面をWatchOSで表示すると、画面が小さいため、全ての項目を表示することができません。そこで、セクション化して情報を格納します。
...
^ レイアウトだけでなく、AIを活用することで、コンテンツ自体の情報量を変化させることができるようになりました。
^ これもまた、段階的なレイアウトのパターンの一つと言えるかなと思います。
...
^ 実際のプロダクトでは、この２つのレイアウトを交互に組み合わせて多様なコンテキストの組み合わせに適応します。
...
^ いくつか見ていきますが、ここでは、同じ情報へのたどり着き方が、画面の広さに応じてどう変わるかに注目してください。
...
^ もし、こう言ったものをカスタムで作るという時に、標準以下のものを作ってもしょうがないわけですね。
^ 膨大なコンテキストの組み合わせに適応する実装というのは、それなりの覚悟を持って臨む必要があることが分かると思います。
...
^ 長時間ありがとうございます。以上になります。ご清聴ありがとうございました。
