...
<!-- 意図: 変わらないものと、環境に合わせて変えるものを判断軸として示す。 -->
...
<!-- 意図: 発表全体の章立てと、各章で扱う内容・時間を案内する。 -->

| 章 | 内容 | 時間 |
|---|---|---:|
| イントロダクション | 自己紹介、デザイン、トークの目的 | 4分 |
| 第1章 柔軟性とは何か | HIG、利用環境、柔軟性の定義 | 5分 |
| 第2章 UIの構造と意味 | 画面・コンポーネント・意味のまとまり、ユースケース、デザインデータ | 13分 |
...
^ 問題の種類が違えば、次に問うべきことも変わります。まず崩れ方を分けて捉えることが、環境に合う表現を考える出発点になります。ここから、この三つを一つずつ見ていきます。
...
^ ここでの問いは、文字をどう小さくするかではありません。文字が読めなくなったとき、固定した高さや行数のどの前提を見直し、どの情報が失われるのかを確認します。
...
^ カードを横に引き伸ばしても、ユーザーが同時に得られる情報や、一覧と詳細の関係が増えるわけではありません。ここでの問いは、広がった空間によって情報密度や情報同士の関係のどこが不足しているのかを特定することです。
...
^ ここでの問いは、画面の比率に合わせてすべてを拡大することではありません。比率の変化によって、画像・タイトル・補助情報のどの関係が崩れたのかを確認します。
...
^ 崩れ方を分けて考えるために、ここからはニュースアプリを共通の題材にします。これらは個別の端末に対応する分岐を増やせば解決するとは限りません。最初から環境の変化を前提に、コンテンツとコンテナの関係を設計する必要があります。
...
<!-- 意図: 柔軟性の中心を、環境が変わってもユーザーの目的を保つこととして定義する。 -->

^ 環境の違いを見てきたので、ここからは、その違いに対応するUIの柔軟性を定義します。HIGにも柔軟性という考え方があります。
...
<!-- 意図: UIの表現を決める三つの要素を、意味・コンテナ・環境として整理する。 -->
...
^ 考慮するのは、コンテナのサイズ、プラットフォームの特性、入力方法、アクセシビリティや利用できる機能です。
...
<!-- 意図: 意味とユースケースを保ち、環境に応じて表現を変える判断へ整理する。 -->

- 変わらないもの：意味とユースケース
- 環境に合わせて変えるもの：表現、レイアウト、操作方法
- 判断する文脈：コンテナとプラットフォーム

^ 同じ意味でも、コンテナや環境が変われば、最適な見た目は変わります。
^ ここで環境を前提にするとは、未来に起こるすべての組み合わせを列挙することではありません。ユーザーの目的と、UIが表現する意味を明確にし、コンテナの制約に対して成立するルールを用意する、という意味です。

---

## 柔軟性は同じ目的を保つこと

<!-- 意図: 柔軟性を、同じ見た目ではなく同じ目的を達成できることとして確認する。 -->
