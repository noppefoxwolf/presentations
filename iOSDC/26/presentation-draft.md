...
- 保つもの：ユーザーの目的と、UIが表現する意味
- 変えるもの：表現、レイアウト、操作方法

^ このトークの目的は、画面サイズや環境が変わっても破綻しないUIの考え方を持ち帰ってもらうことです。
^ そのために、ユーザーの目的とUIが表現する意味は保ち、表現、レイアウト、操作方法は環境に合わせて変える、という順番で見ていきます。
...
^ ただし、最初からすべてを同時に設計するわけではありません。まず小さな画面を起点にし、そこから幅や環境を変えたときに何が起きるかを見ていきます。
^ ひとつの対象を共通の題材として、どの判断を共通化し、どこで表現を変えるかに注目してください。
...
^ 最初からiPhoneの縦向きの画面だけを基準にしてUIを設計すると、環境が変わったときに問題が起こります。
...
^ 今日の題材としてニュースアプリを使うのは、一覧、記事、媒体、保存、読み上げなど、同じ意味から複数のユースケースを考えやすいからです。実装済みのアプリを完成させる話ではなく、設計判断を具体的にするための共通の例として使います。
