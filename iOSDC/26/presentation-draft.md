...
^ よろしくお願いします。今日は「柔軟性のあるUIで、多くの環境でアプリを活躍させる」をテーマに、まずは環境に応じてUIを設計する考え方からお話しします。
...
^ noppeと言います。
^ 個人開発でMastodon向けのクライアントアプリを作ったり、iOSのUI設計や実装に関わっています。

---

## 柔軟なUIの基本

<!-- 意図: 環境が変わってもUIを成立させる問題を提示し、後の診断と設計の考え方へつなぐ。 -->

- 環境が変わっても、UIを成立させるには？
- 画面サイズ・向き・入力方法が変わると、何が起こるのか？
- 変化する条件の中で、ユーザーが目的を達成し続けるには？

^ 画面サイズや環境が変わっても、UIを成立させ続けるにはどうすればよいのでしょうか。画面サイズ・向き・入力方法が変わると、今のUIのどこに問題が起こるのかを考えます。
^ AppleのHuman Interface Guidelines（HIG）でも、環境に応じたUI設計が扱われています。環境が変わってもUIを成立させることは、特定の画面だけの問題ではありません。
...
^ 同じ機能でも、使われる場所によって必要な画面の広さ、操作方法、情報密度が変わります。まずは環境の違いが、UIのどこに影響するのかを見ていきます。
