...
^ こんにちは、PocochaでiOSアプリの設計と実装に携わっているnoppeです。今日は「柔軟性のあるUIで、多くの環境でアプリを活躍させる」をテーマに、まずは環境に応じてUIを設計する考え方からお話しします。
...
^ まず環境が変わるとUIに何が起きるかを確認します。そこから意味の整理、サイズ適応、構成変更、意味を表すAPIの順に見ていきます。具体的な題材は、考え方を整理したあとで紹介します。
...
^ 改めまして、PocochaでiOSアプリの設計と実装に携わっているnoppeです。
...
^ このトークでは、ひとつの対象を共通の題材として、環境に応じてサイズ、配置、情報量、操作方法を変えて表現します。最初は小さな画面から始め、最後には広い画面や異なるプラットフォームまで同じ意味を展開します。題材そのものより、どの判断を共通化し、どこで表現を変えるかに注目してください。
...
^ まずUIの構造と、UIが表現する意味を整理します。次に、同じ構成のままサイズへ連続的に適応する方法を見ます。その後、タブバーをサイドバーに変えるような構成変更と、異なるコンテキストでの表現を扱い、最後に適応するコンポーネントとAPIへつなげます。
