...
^ こんにちは、Pocochaのデザインエンジニアのnoppeです。今日は「柔軟性のあるUIで、多くの環境でアプリを活躍させる」をテーマに、ニュースを共通の例としてお話しします。
...
^ まずニュースを題材に、環境が変わると何が起きるかを確認します。そこからモデル、サイズ適応、構成変更、意味を表すAPIの順に見ていきます。
...
^ では、まず環境が変わるとUIにどんな問題が起きるのか、ニュースの例で見ていきます。

## 目的説明

^ 画面サイズや環境が変わっても破綻しないUIの設計を、ニュースの例で段階的に見ていきます。
...
<!-- 編集メモ：本編外の補助候補。HIGのFlexibility、モデル×コンテナ×環境、plain/grouped list、モデルとデータの用語差。話者ノートにはしない。 -->
