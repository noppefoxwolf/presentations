...
^ こんにちは、Pocochaのデザインエンジニアのnoppeです。今日は「柔軟性のあるUIで、多くの環境でアプリを活躍させる」をテーマに、ニュースアプリを共通の例としてお話しします。
...
^ まずニュースアプリを題材に、環境が変わると何が起きるかを確認します。そこからモデル、サイズ適応、構成変更、意味を表すAPIの順に見ていきます。
...
^ 今日は、画面サイズや環境が変わっても破綻しないUIの設計を、ニュースアプリの例で段階的に見ていきます。先ほど紹介したニュースアプリを使い、同じモデルが異なる表現へ変化する過程を確認します。
...
^ このトークでは、ニュースアプリという同じ題材を使い、環境に応じてサイズ、配置、情報量、操作方法を変えて表現します。最初はiPhoneのニュース記事一覧から始め、最後にはiPadやWidget、Apple Watchまで同じモデルを展開します。
