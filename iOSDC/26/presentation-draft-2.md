...
^ 環境が変わったときに、何を確認し、どこで実装するかを、この5つの流れに沿って見ていきます。
...
^ ここでも、モデルやコンポーネントの設計にはまだ進みません。まず、環境が変わってもユーザーが達成したいことを言葉にします。
^ 具体的な例を見てみましょう。
...
^ 三つ目の判断では、第2章で決めたユーザーの目的を、モデルと見せ方へ落とし込みます。
^ まず、画面やコンポーネントを、役割を持つ単位として捉えます。

---

# 3.1 UIを組み替えられる単位で捉える

^ まず、画面を目的と役割を持つ部品に分けます。
^ どこまでを一つの部品として扱うかを決めることで、環境に合わせて組み替えられるようになります。
...
<!-- 3章あたりから急にニュースの例じゃなくてモデルから画面作る話になっててわかりにくいので直したい -->
...
^ 画面を役割のある単位に分けたら、次は、その中心となる対象をモデルとして定義します。
^ ここからは、モデルを基準に、環境に応じてどの情報を見せるかを決めます。ここではニュース記事をArticleとして例にします。
...
^ ここまでで、Articleとユーザーの目的、それに応じた見せ方を決めました。
^ 次は、環境が変わったときに、その見せ方をどの範囲まで調整するかを選びます。
...
^ 最後の判断は、環境ごとの違いをどこで吸収するかです。呼び出し側はArticleとユースケースを渡し、環境に合う見せ方はコンポーネント側で選びます。
