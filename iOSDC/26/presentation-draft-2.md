...
^ iPhoneで動く想定しかしていないUIは別の環境で使いにくくなります。
...
^ すぐに具体的な対策をする前に、iPhone向けの画面で何を前提にしていたのかを確認します。
^ このように広い環境へ移すと、それまで意識していなかった前提が見えてきます。
...
^ 「一覧は一列」という実装が、広い環境では使いにくさとして現れました。
...
^ 環境が変わっても役割を保てる単位へ、画面を分けるところから始めます。

---

# 3.1 UIを組み替えられる単位で捉える

^ 一覧を別の構成にも組み替えられるように、画面を目的と役割を持つ部品に分けます。
...
^ 次は、その判断をコンテナと環境へ重ね、同じ構成で調整するか、構成まで変えるかを選びます。
...
^ 第4章では、ここまでに決めた判断を、実際に置かれるコンテナと環境に当てはめます。
...
^ 例えばニュース一覧を広いウインドウへ移したとします。使いにくいのは、カードが横へ伸びても比較できる記事が増えないことです。ユーザーは記事を見つけ、読むか判断したい。Articleでは見出しや媒体名を残して記事を識別できるようにし、カード幅を保って列数を増やします。その選択をArticleViewやGridの中で行います。このように、問題、ユースケース、モデルと情報、変更範囲、吸収する場所を順に決めます。
^ この順番なら、使いたいAPIに問題を合わせるのではなく、実際に困っていることに合う方法を選べます。デザインレビューでも、「この要約がないと何を判断できないか」「広いときに一覧を残すと何がしやすくなるか」と、同じ順番で情報の優先順位を確認できます。
^ すべてのAPI名を覚えなくても、この5つの質問から実装を調べ始められます。

---

# 環境に応じて見せ方を選ぶルールを作る

^ 柔軟なUIを作るのは、すべての環境で同じ見た目にすることではありません。Articleとユースケースを共通にし、置かれた場所に合う表現を選びます。環境ごとの差は、コンポーネントとAPIの中へ集めます。
^ 新しい環境が増えても、画面ごとに分岐を足すのではなく、この5つの質問に答えながら、必要な表現と選択ルールを加えられます。
^ 次に新しい画面サイズへ対応するときは、最初にデバイス名のifを書く前に、このUIは何を表し、ユーザーは何をしたいのかを一度考えてみてください。変えたくないことが分かれば、環境に合わせて変えてよいところも見つけやすくなります。ありがとうございました。
