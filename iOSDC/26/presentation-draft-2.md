...
^ ニュース一覧を、記事を表示するArticleViewやフィルターといった役割を持つ単位の組み合わせとして捉えると、同じArticleを一列にもグリッドにも並べられます。
^ Viewを分割するときは、別の構成でも役割が残る単位を選びます。
^ 分割の目的は、コードの行数を減らすことでも、すべてを再利用することでもありません。一覧を二列にしても詳細と並べても役割が変わらない部分を、独立したコンポーネントにします。ArticleViewが独立していれば、一列にもグリッドにも詳細の関連項目にも置けます。
...
^ 例えばArticleViewは、Articleを表示し、読むか判断し、詳細へ進むための部品です。この役割が決まると、カード以外の表現も選べます。
^ 例えば画像・見出し・媒体名を別々のViewとして公開すると、呼び出し側が毎回その組み合わせを決めることになります。すると、一覧と詳細でArticleの見せ方がばらばらになります。
^ 再利用したいのは部品の数ではなく、Articleを識別して開くという役割です。一番小さなViewまで分けるのではなく、ArticleViewとして扱いたいまとまりを一つの境界にします。
...
^ ArticleViewのような役割のある部品を決めたら、次は、その中心となるArticleをモデルとして定義します。
^ ここからは、モデルを基準に、環境に応じてどの情報を見せるかを決めます。
...
^ 寸法を調整しても、役割の異なる内容を同じ移動にまとめると扱いにくくなることがあります。
...
^ 表示場所ごとに選ぶArticle、情報、操作を、呼び出し側へ散らさないようにします。
^ そのため実装は、モデル、表示バリエーション、選択、プラットフォームの表現という層に分けます。
...
^ 先ほど構成変更の例として挙げた補助情報用の領域を、ここではInspectorというAPIで表します。環境ごとの差を呼び出し側へ漏らさない設計です。
...
^ 例えばニュース一覧を広いウインドウへ移したとします。使いにくいのは、カードが横へ伸びても比較できる記事が増えないことです。
^ ユーザーが記事を見つけ、読むか判断したいなら、Articleには見出しや媒体名を残し、ArticleViewではカード幅を保って列数を増やします。
^ このように、問題、ユースケース、モデルと情報、変更範囲、吸収する場所を順に決めます。
^ この順番なら、使いたいAPIに問題を合わせるのではなく、実際に困っていることに合う方法を選べます。
^ デザインレビューでも、同じ順番で情報の優先順位を確認できます。「この要約がないと何を判断できないか」や、「一覧を残すと何がしやすくなるか」を考えます。
...
^ 柔軟なUIを作るのは、すべての環境で同じ見た目にすることではありません。Articleモデルとユースケースを共通にし、置かれた場所に合う表現を選びます。環境ごとの差は、ArticleViewと選択APIの中へ集めます。
