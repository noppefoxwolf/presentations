...
^ この優先順位が決まると、次に同じモデルから、環境に合わせた複数の見せ方を用意できます。

---

# 4. レイアウトを調整するか、構成を変えるか

^ ここまでで、モデルとユーザーの目的、それに応じた見せ方を決めました。
^ 次は、環境が変わったときに、その見せ方をどの範囲まで調整するかを選びます。

---

# 問題・目的・モデルを、見せ方の判断につなげる
...
^ 1章で、何が使いにくくなったかを特定しました。2章で、環境が変わっても保ちたいユーザーの目的を決めました。3章では、その目的を支えるモデルと情報の優先順位を整理しました。
^ ここからは、この三つを組み合わせて、コンテナと環境に合わせた見せ方を選びます。
...
```mermaid
flowchart TB
    Problem["1章：問題<br/>何が使いにくいか"] --> Decide["4章：見せ方を選ぶ"]
    Purpose["2章：目的<br/>ユーザーは何をしたいか"] --> Decide
    Model["3章：モデルと情報<br/>何を表し、何を残すか"] --> Decide
    Context["コンテナと環境<br/>どこで使われるか"] --> Decide
    Decide --> Layout["レイアウト調整"]
    Decide --> Structure["構成変更"]
```

^ 1章で、何が使いにくくなったかを特定しました。2章で、ユーザーが何をしたいかを確認し、3章で、何をモデルとして扱い、どの情報を残すかを決めました。
^ ここからは、その判断を、実際に置かれるコンテナと環境へ組み合わせます。
