...
^ 本文のように一つの流れとして読むものと、一覧や設定のように役割が違うものを、同じスクロールへ無理に入れないようにします。
...
^ 役割の違うコンポーネントや、その環境に固有の操作まで、無理に同じViewへ押し込むと条件分岐が増えます。
...
^ デザインレビューでも、同じ順番で情報の優先順位を確認できます。例えば、「この要約がないと何を判断できないか」を考えます。広いときには、「一覧を残すと何がしやすくなるか」も確認できます。
...
^ 変えたくないことが分かれば、環境に合わせて変えてよいところも見つけやすくなります。ありがとうございました。
