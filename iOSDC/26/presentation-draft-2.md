...
^ 画面の形だけでなく、利用できる空間や操作方法まで変わると、ひとつの前提に合わせたUIは別の環境で使いにくくなります。
^ 今回のトークは、実体験を元にしています。アプリを複数のデバイスやウインドウへ広げるうちに、画面ごとの分岐を足すほど、次の環境へ展開しにくくなると感じました。
...
^ ここで起きているのは、必要な情報が欠けて、記事を識別しにくくなることです。
...
| 関連する情報が離れて見える | UIの構成変更 |

^ ここまでの三つの問題は、どれも「環境に合わせる」だけでは同じ対応になりません。
^ 必要な情報が欠けているなら、内容に合わせてサイズや情報量を調整します。空間を活かせないなら、読みやすい幅を保ちながら同時に扱う情報を増やします。情報の関係が崩れているなら、画面の構成まで見直します。
^ ただし、何を変えてもよいかは、ユーザーがそのUIで何をしたいかによって決まります。次に、どの環境でも守るユースケースを確認します。
...
^ 再利用したいのは部品の数ではなく、Articleを識別して開くという役割です。一番小さなViewまで分けるのではなく、Articleとして扱いたいまとまりを一つの境界にします。
