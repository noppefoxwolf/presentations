...
^ 今回のトークはひとつのUIを、iPhone、iPad、Macなど、さまざまな表示先で使いやすくするということを考えていきます。
^ 特にiPadではウインドウを自由にリサイズでき、同じアプリの表示領域が狭くも広くもなります。また、WidgetやWatchへ機能を届けることもあります。
^ iPhoneで動く想定しかしていないUIは、別の表示先で使いにくくなります。
...
^ このように表示先が広がると、それまで意識していなかった前提が見えてきます。
...
^ どの表示先でも変えたくないのは、表示する記事、ユーザーができること、選択や保存の状態です。
^ ニュースアプリなら、どの表示先でも記事を探し、読み、保存できるようにします。
^ それを使いやすく届けるために、一度に見せる情報、レイアウト、操作方法は表示条件に合わせます。この区別ができると、端末ごとに画面を作り直す代わりに、見せ方を選ぶルールを設計できます。
...
^ コンテナやプラットフォームが変わったときに、何を確認し、どこで実装するかを、この5つの流れに沿って見ていきます。
^ まずは、どの部分が使いにくくなったかを特定し、次にUIが何を表し、ユーザーが何をしたいかを確認します。それを元にレイアウト調整か構成変更かを選び、コンテナとプラットフォームの差を吸収する場所まで決めると、分岐を広げずに実装できます。
