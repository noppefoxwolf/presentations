...
^ このご時世では、席に来てもらってデバイスに直接転送すると言ったことは出来ませんから、この回数はなるべく抑えたい箇所になります。
...
^ つまりデバッグメニューは、環境の参照だけではなく変更も行えるということなんですね。まさしく神モードです。
...
^ 再起動が必要なものがあればその旨を表示してアプリを落としてあげたり、強い権限のものは色を変えたりしましょう。
^ そして、機能を整頓するのも重要です。
...
^ アプリからアプリを再起動するには、AppleのShortcutアプリを利用します。
...
^ パラメータ変更のUI実装はそこそこ実装コストがかかるので、アプリの肝になる部分に絞って行うと良いと思います。
...
^ 例えば、AppleのFindMyアプリではAirTagの探索をする画面にデバッグメニューが存在していてユーザーが呼び出せる時期がありました。
^ ここでは画面内のカメラ解像度やブラー濃度が調整できるようになっていて、実際の動作を見ながらフィールを改善していたと推測できます。
^ またアニメーションに関してもAirTag無しで実行できるAutoUITestがあるなど、物理的な移動を省略して最適化を行っていった痕跡が見て取れます。
^ 余談ですが、このように時々デバッグメニューは本番の環境で呼べてしまう事があるので触ってみると、どんな機能が実際の現場で検証されているのかがわかって良い勉強になります。
...
^ ランタイムデバッグは先ほど解説した通り、フィールを検証する側面が強いと言えます。
...
^ そして、定量的な指標の見える化をして、共通認識を高めて費用対効果を上げていきます。
...
「重い・遅い」など、定性的な意見はログを見ても分からないことが多い。
`RPScreenRecorder`を使うことで操作の15秒前までを記録することができる

^ 定性的な意見というのは、裏付けを取ることで改修コストがグッと下がります。
^ そこで直前の操作を映像で振り返ることで、定性的な意見の言語化を促すことにしましょう。
^ ビデオカメラを用意して操作している様子を録画しても良いのですが、ここではランタイムデバッグらしくアプリで完結させてみます。
^ iOS15から使えるReplayKitのクリップバッファリングを使えば、録画停止から15秒前までの映像を記録する事が出来そうです。
...
^ これを利用して、テスターは問題が起きてからレポートボタンを押すだけで、例え小さな違和感であったとしてもレポートする事が出来ます。
^ RPScreenRecorder自体はビデオを書き出すところまでしかしてくれませんが、チケットを切るところまでをアプリ内で出来ると効率的に時間を使う事が出来そうですね。
...
^ テスターによっては、細かい問題に気がつかないケースもあります。
^ 例えば、フレームレートが60を維持するようにと言われても、ランタイムデバッグでそれが達成されているのかは分かりにくいものです。
^ アプリの品質指標が明確にある場合、指標の見える化をすることで問題の見過ごしを防ぐ事が出来ます。
^ 右のスクリーンショットのように、画面上に現在のパフォーマンスと指標を表示しておくことで指標を超えたかどうかが分かります。
^ これをきっかけにInstrumentsで詳細に調べ上げるのも良いでしょう。
...
^ また、ランタイムデバッグは定量的な指標を追い難いという問題がありましたが、現在のパフォーマンスを画面上にオーバーレイすることで、指標の見える化をし、テスターの感覚に依存しない体制を作る事が出来ました。
