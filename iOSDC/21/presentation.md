...
^ あとこの収録の2週間前に子供が産まれました。
^ 今日話す開発テクニックは、Pocochaとvearの両方で採用しているものなので、小さなアプリから大きなアプリまで参考になると思います。
...
^ また、注意して欲しいのですが、今回のセッションではランタイムデバッグが開発においてどのように作用するのかという話をしますが、これはテスト書くのをやめて実機デバッグに一本化しようぜ。という話ではないので、あくまで実機デバッグせざる得ないケース。
...
^ では早速初めて行くのですが、皆さんどんな時にランタイムデバッグをしているでしょうか。
^ 開発フローを振り返ると、ランタイムデバッグがアプリ開発の中の多くのステップで実施されていることが分かります。
^ 今見えているのは、開発からリリースまでのステップを簡易的に分けたものです。
^ [click]例えば、アプリの開発フローの中で、最初にランタイムデバッグを行うのはコンパイル直後の3番のステップです。
...
^ 今見えているのは、ランタイムデバッグの目的である動作確認に辿り着くまでのステップになります。
^ [click]コンパイルや転送の高速化は他のセッションでも話されている議題かと思いますので、ここでは3番以降をランタイムデバッグを上手に使って改善をしてみます。
...
^ 詳細を表示する画面が必要な場合は別で用意しつつ、ダッシュボード的に重要な情報に絞った画面を用意しましょう。テスター向けの画面でもUIUXを意識することが大切です。
...
^ [Click]通常のアプリ開発と同じように、ViewControllerをpresentするだけです。
^ [Click]ハックされたiPhoneでは直接関数が呼び出せるため、このようにDEBUGフラグを使って実行バイナリから実装を取り除く必要があります。
...
^ さて、次にランタイムデバッグにおいてトレードオフになる感覚のブレについて解説します。

---

## 人による感覚のブレ

UnitTestと異なり人間が触る以上、感覚の違いが起こりうる
定性的な情報が集められる反面、人によって報告の基準はまちまち
また、報告されたものを改善するコストが高く、自然と意見が集まりにくくなってしまう

^ ランタイムデバッグは先ほど言った通り、フィールを検証する側面が強いと言えます。
^ それは定性的な意見が得られるというメリットがある反面、問題として報告するかの基準が定まらないとも言えます。
^ また、報告しても定性的な問題の検証は非常に時間がかかるわりには改善が見えにくく、自然と意見が集まりにくくなってしまう側面もあります。
...
^ そして、定量的な指標の見える化をする事で、共通認識を高めることで解決します。
...
^ アプリの品質指標が明確にある場合、指標の見える化をすることで品質の見過ごしを防ぐ事が出来ます。
...
^ ここで超えたものをInstrumentsなどで詳細に分析することで、細かな改善を素早く行えます。
...
^ DebugMenuはアイテムの検索や、直近使ったアイテムの履歴も出るので、すぐにメニューを見つける事が出来ます。
...
^ このようなDebugItemをはじめ、DebugMenuはデバッグメニューにありがちなUIを提供しています。
...
^ アクションはタップ時や、トグルの変更時、スライダーの変更時などが選べます。
...
^ まとめです。
^ ランタイムデバッグは時間的・金銭的なコストがかかる一方で定性的な意見が引き出せるという事がわかりました。
